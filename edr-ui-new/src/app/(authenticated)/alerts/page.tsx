...
function ProcessTreeSection({ events }: { events: Event[] }) {
  const [tree, setTree] = useState<ProcessNode[] | null>(null);
  const [loading, setLoading] = useState(false);
  const [expanded, setExpanded] = useState(false);

  // Extract PID and agent_id from first process event
  const processEvent = events.find(
    (e) => e.event_type === "PROCESS_EXEC" || e.event_type === "PROCESS_FORK"
  );
  const pid = processEvent?.payload?.process
    ? (processEvent.payload.process as Record<string, unknown>).pid as number
    : (processEvent?.payload?.pid as number) ?? null;
  const agentId = processEvent?.agent_id;
    ...
    <div>
      <div className="flex items-center gap-2 mb-2">
        ...
        {!expanded && (
          ...
          >
            ...
            {loading ? "Loading..." : `View Process Tree (PID ${pid})`}
          </button>
        )}
      </div>
      ...
      {expanded && tree && tree.length === 0 && (
        <p className="text-xs" style={{ color: "var(--muted)" }}>
          No process tree data available for PID {pid}.
        </p>
      )}
    </div>
  );
      ...
      <div className="p-4 space-y-5">
        ...
        <div>
          ...
          <button
            ...
            disabled={explaining}
            className="flex items-center gap-1.5 rounded border px-3 py-1.5 text-xs font-medium transition-colors hover:bg-[var(--surface-2)] disabled:opacity-50"
            style={{
              background: "var(--surface-1)",
              borderColor: "var(--border)",
              color: "var(--primary)",
          ...
          >
            ...
            {explaining ? "Analyzing alert..." : explanation ? "Re-analyze" : "Explain with AI"}
          </button>
          {explanation && (
            <div
              className="mt-2 rounded border p-3 text-xs leading-relaxed whitespace-pre-wrap"
        ...
        {/* Process Tree */}
        <ProcessTreeSection events={relatedEvents ?? []} />
