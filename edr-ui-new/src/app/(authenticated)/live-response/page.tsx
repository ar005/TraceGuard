...
export default function LiveResponsePage() {
  ...
  async function submitCommand() {
    ...
    try {
      const res = await api.post<{ output?: string; result?: string; error?: string }>(
        "/api/v1/liveresponse/command",
        { agent_id: selectedAgentId, command: cmd }
      );
      setOutputs((prev) => [
        ...prev,
        {
          id: blockId,
          command: cmd,
          response: res?.output ?? res?.result ?? res?.error ?? "Command sent (no output returned)",
