...
// gRPC transport — streams events to EDR backend. OS set to "windows".
...
// TaskInstruction mirrors proto.TaskInstruction — kept local to avoid
// importing the backend proto package from the agent.
...
// TaskResult is queued by the executor and flushed on the next heartbeat.
...
func (t *GRPCTransport) SetContainment(c ContainmentController) { t.containment = c }
func (t *GRPCTransport) OnConfigChange(fn func(newVersion string)) { t.onConfigChange = fn }

// OnTask registers a callback invoked for each task delivered by the backend.
func (t *GRPCTransport) OnTask(fn func(TaskInstruction)) { t.onTask = fn }
	...
	for {
		...
		case <-ticker.C:
			...
			err := conn.Invoke(hbCtx, methodHeartbeat, &heartbeatRequest{
				...
				Timestamp: time.Now().UnixNano(), OS: "windows",
				TaskResults: results,
