...
import (
	...
	intyara "github.com/youredr/edr-agent-win/internal/yara"
...
// execExtensions are file extensions that trigger a YARA scan (Windows-centric).
var execExtensions = map[string]bool{
	".exe": true, ".dll": true, ".sys": true, ".scr": true,
	".ps1": true, ".psm1": true, ".psd1": true,
	".vbs": true, ".vbe": true,
	".js":  true, ".jse": true,
	".bat": true, ".cmd": true,
	".hta": true, ".msi": true, ".msp": true, ".msc": true,
	".cpl": true, ".jar": true,
}

// execMagics are byte-level file magic signatures that trigger a YARA scan.
var execMagics = [][]byte{
	{0x4D, 0x5A},             // MZ — PE/DLL/EXE/SCR
	{0x50, 0x4B, 0x03, 0x04}, // ZIP/JAR
	{'#', '!'},               // shebang (PS1 wrappers)
}
...
// Start begins rule fetching and subscribes to file events. Returns immediately.
func (m *Monitor) Start(ctx context.Context) error {
	...
	// Periodic rule refresh goroutine.
	m.wg.Add(1)
	go func() {
		defer m.wg.Done()
	...
	unsub1 := m.bus.Subscribe("FILE_CREATE", func(ev events.Event) {
	...
	unsub2 := m.bus.Subscribe("FILE_WRITE", func(ev events.Event) {
		fe, ok := ev.(*types.FileEvent)
		if !ok {
			return
		}
	...
	m.log.Info().Int("workers", m.cfg.WorkerCount).Msg("YARA scanner started")
	return nil
}
...
func (m *Monitor) Stop() {
	...
	m.wg.Wait()
	m.log.Info().Msg("YARA scanner stopped")
}
...
func (m *Monitor) shouldScan(path string) bool {
	...
	// No matching extension — check magic bytes (read first 4 bytes).
	f, err := os.Open(path)
	if err != nil {
		return false
	}
