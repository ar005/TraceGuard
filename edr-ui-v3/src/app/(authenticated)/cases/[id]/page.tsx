...
const STATUS_STYLE: Record<string, string> = {
  ...
  CLOSED:        "bg-neutral-700 text-neutral-400 border-neutral-600",
};
      ...
      {editing ? (
        <div className="flex items-center gap-1 mt-0.5">
          <input value={val} onChange={(e) => setVal(e.target.value)}
            className="flex-1 bg-neutral-800 border border-neutral-600 rounded px-2 py-1 text-sm" />
          <button onClick={save} disabled={saving} className="p-1 text-emerald-400 hover:text-emerald-300">
            <Check size={14} />
          </button>
          <button onClick={() => { setVal(value); setEditing(false); }} className="p-1 text-neutral-500 hover:text-neutral-300">
            <X size={14} />
          </button>
        </div>
      ) : (
        <div className="flex items-center gap-1 group mt-0.5">
          <span className="text-sm">{value || <span className="text-neutral-600 italic">none</span>}</span>
  ...
  return (
    <div className="space-y-2">
      <textarea
        ...
        rows={3}
        className="w-full bg-neutral-800 border border-neutral-700 rounded-lg px-3 py-2 text-sm resize-none focus:border-neutral-500 outline-none"
      />
      <div className="flex justify-end">
        <button onClick={submit} disabled={saving || !body.trim()}
          className="px-3 py-1.5 text-sm rounded bg-cyan-600 hover:bg-cyan-500 disabled:opacity-40 font-medium flex items-center gap-1">
...
function LinkAlertModal({ caseID, onLinked, onClose }: { caseID: string; onLinked: () => void; onClose: () => void }) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 p-4">
      <div className="bg-neutral-900 border border-neutral-700 rounded-xl w-full max-w-sm">
        ...
        <div className="p-5 space-y-3">
          ...
          <div>
            <label className="text-xs text-neutral-400 mb-1 block">Alert ID</label>
            <input value={alertID} onChange={(e) => setAlertID(e.target.value)} autoFocus placeholder="alert-…"
...
export default function CaseDetailPage() {
  ...
  const fetchCase = useCallback(
    () => api.get<CaseDetail>(`/api/v1/cases/${id}`),
    [id]
  );
  ...
  if (error || !cs) return <div className="p-6 text-sm text-red-400">{error ?? "Case not found"}</div>;

  return (
    <div className="p-6 space-y-6">
      {/* Header */}
      <div className="flex items-start gap-3">
        <Link href="/cases" className="mt-0.5 text-neutral-500 hover:text-neutral-300">
          <ArrowLeft size={18} />
        </Link>
        <div className="flex-1 min-w-0">
          <div className="flex items-center gap-2 flex-wrap">
            <h1 className="text-lg font-semibold">{cs.title}</h1>
            <span className={`inline-block px-2 py-0.5 rounded text-xs font-medium border ${STATUS_STYLE[cs.status] ?? "bg-neutral-800 text-neutral-400 border-neutral-700"}`}>
              {cs.status}
            </span>
          </div>
          {cs.description && <p className="text-sm text-neutral-400 mt-1">{cs.description}</p>}
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="lg:col-span-2 space-y-5">
          ...
          <div className="space-y-3">
            ...
            {narrative && (
              <div className="rounded-lg border border-violet-500/20 bg-violet-500/5 p-3">
                ...
                <p className="text-sm text-neutral-300 leading-relaxed whitespace-pre-wrap">{narrative}</p>
              </div>
            )}
            <NoteComposer caseID={id} onAdded={refetch} />
            {notes.length === 0 && (
              <p className="text-xs text-neutral-600 text-center py-4">No notes yet.</p>
            )}
            <div className="space-y-3">
              {[...notes].reverse().map((note) => (
                <div key={note.id} className="bg-neutral-900 border border-neutral-800 rounded-xl p-4 group">
                  <div className="flex items-center justify-between mb-2">
                    <span className="text-xs text-neutral-500">
                      <span className="text-neutral-400 font-medium">{note.author || "system"}</span>
          ...
          <div className="space-y-3">
            <div className="flex items-center justify-between">
              <h2 className="text-sm font-semibold text-neutral-300">Linked Alerts ({alerts.length})</h2>
            ...
            {alerts.length === 0 && (
              <p className="text-xs text-neutral-600 text-center py-4">No alerts linked yet.</p>
            )}
            <div className="space-y-2">
              {alerts.map((a) => (
                <div key={a.id} className="flex items-center gap-3 bg-neutral-900 border border-neutral-800 rounded-lg px-3 py-2">
                  <span className={`w-1.5 h-1.5 rounded-full shrink-0 ${severityBgClass(a.severity)}`} />
                  <div className="flex-1 min-w-0">
                    <Link href={`/alerts?id=${a.id}`} className="text-sm font-medium hover:text-cyan-300 transition-colors truncate block">
                      {a.title}
                    </Link>
                    <span className="text-xs text-neutral-500">{a.hostname} · {a.rule_name} · {timeAgo(a.last_seen)}</span>
                  </div>
                  <span className="text-xs text-neutral-500">{severityLabel(a.severity)}</span>
                  <button onClick={() => unlinkAlert(a.id)}
                    className="p-1 text-neutral-600 hover:text-red-400 shrink-0">
                    <X size={13} />
                  </button>
                </div>
              ))}
        ...
        <div className="space-y-4">
          <div className="bg-neutral-900 border border-neutral-800 rounded-xl p-4 space-y-4">
            <h2 className="text-xs font-semibold text-neutral-400 uppercase tracking-wider">Details</h2>
                ...
                {STATUSES.map((s) => (
                  <button key={s}
                    ...
                    className={`px-2 py-0.5 rounded text-xs font-medium border transition-colors ${
                      cs.status === s
                        ? STATUS_STYLE[s] ?? "bg-neutral-700 text-white border-neutral-600"
            ...
            <div>
              <span className="text-xs text-neutral-500 block mb-1">Severity</span>
            ...
            <div>
              <span className="text-xs text-neutral-500 block mb-1">MITRE ATT&amp;CK</span>
              <div className="flex flex-wrap gap-1">
                {(cs.mitre_ids ?? []).map((m) => (
                  <span key={m} className="px-1.5 py-0.5 rounded bg-neutral-800 text-xs font-mono text-purple-400">{m}</span>
                ))}
                {(cs.mitre_ids ?? []).length === 0 && <span className="text-xs text-neutral-600 italic">none</span>}
              </div>
            </div>

            <div>
              <span className="text-xs text-neutral-500 block mb-1">Tags</span>
              <div className="flex flex-wrap gap-1">
                {(cs.tags ?? []).map((t) => (
                  <span key={t} className="px-1.5 py-0.5 rounded bg-neutral-800 text-xs text-neutral-300">{t}</span>
                ))}
                {(cs.tags ?? []).length === 0 && <span className="text-xs text-neutral-600 italic">none</span>}
              </div>
            </div>

            <div className="pt-2 border-t border-neutral-800 text-xs text-neutral-500 space-y-1">
              <div>Created by <span className="text-neutral-400">{cs.created_by || "system"}</span></div>
              <div>Opened {timeAgo(cs.created_at)}</div>
              {cs.closed_at && <div>Closed {timeAgo(cs.closed_at)}</div>}
              <div className="font-mono text-neutral-600 truncate">{cs.id}</div>
            </div>
          </div>
        </div>
      </div>
