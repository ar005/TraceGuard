...
func (c *Config) Validate() error {
	...
	if c.Monitors.YARA.Enabled && c.Monitors.YARA.WorkerCount <= 0 {
		errs = append(errs, fmt.Sprintf("monitors.yara.worker_count must be > 0 when yara is enabled (got %d)", c.Monitors.YARA.WorkerCount))
	}
