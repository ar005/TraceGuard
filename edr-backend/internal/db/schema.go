...
var migrations = []struct {
	...
	{
		name: "add_sequence_rule_fields",
		sql: `
		ALTER TABLE rules ADD COLUMN IF NOT EXISTS sequence_steps    JSONB   NOT NULL DEFAULT '[]';
		ALTER TABLE rules ADD COLUMN IF NOT EXISTS sequence_window_s INT     NOT NULL DEFAULT 30;
		ALTER TABLE rules ADD COLUMN IF NOT EXISTS sequence_by       TEXT    NOT NULL DEFAULT 'agent_id';
		`,
	},
	{
		name: "seed_sequence_rules",
		sql: `
		INSERT INTO rules (id, name, description, severity, event_types, conditions, mitre_ids, author,
		                   rule_type, threshold_count, threshold_window_s, group_by,
		...
		(
			'rule-seq-shell-net',
			'Shell Spawned Then Outbound Connection (sequence)',
			'A shell was spawned and then an outbound network connection was made within 30 seconds — possible reverse shell or C2.',
			4,
			ARRAY['PROCESS_EXEC','NET_CONNECT'],
			'[]',
			ARRAY['T1059.004','T1071.001'],
			'system',
			'sequence', 0, 0, '',
			'[{"event_types":["PROCESS_EXEC"],"conditions":[{"field":"process.comm","op":"in","value":["bash","sh","dash","zsh","fish","ksh"]}],"label":"shell spawned"},{"event_types":["NET_CONNECT"],"conditions":[{"field":"direction","op":"eq","value":"OUTBOUND"},{"field":"is_private","op":"eq","value":false}],"label":"outbound connection"}]',
			30,
			'agent_id'
		),
		(
			'rule-seq-download-exec',
			'File Downloaded Then Executed (sequence)',
			'A download tool fetched a file, then a new process was executed within 60 seconds — possible dropper/payload delivery.',
			4,
			ARRAY['PROCESS_EXEC'],
			'[]',
			ARRAY['T1105','T1204.002'],
			'system',
			'sequence', 0, 0, '',
			'[{"event_types":["PROCESS_EXEC"],"conditions":[{"field":"process.comm","op":"in","value":["curl","wget","fetch","aria2c"]}],"label":"download tool executed"},{"event_types":["PROCESS_EXEC"],"conditions":[{"field":"process.exe_path","op":"startswith","value":"/tmp/"}],"label":"binary executed from /tmp"}]',
			60,
			'agent_id'
		),
		(
			'rule-seq-privesc-persist',
			'Privilege Escalation Then Persistence (sequence)',
			'A sudo/su command was followed by cron or systemd modification within 120 seconds — possible privilege escalation with persistence setup.',
			4,
			ARRAY['SUDO_EXEC','CRON_MODIFY'],
			'[]',
			ARRAY['T1053.003','T1548.003'],
			'system',
			'sequence', 0, 0, '',
			'[{"event_types":["SUDO_EXEC"],"conditions":[],"label":"privilege escalation via sudo"},{"event_types":["CRON_MODIFY"],"conditions":[{"field":"suspicious","op":"eq","value":true}],"label":"suspicious cron modification"}]',
			120,
			'agent_id'
		),
		(
			'rule-seq-cred-access-exfil',
			'Credential File Read Then Network Connection (sequence)',
			'A sensitive file (shadow, passwd, SSH keys) was accessed, then an outbound connection was made within 60 seconds — possible credential theft.',
			4,
			ARRAY['FILE_CREATE','FILE_WRITE','NET_CONNECT'],
			'[]',
			ARRAY['T1003.008','T1041'],
			'system',
			'sequence', 0, 0, '',
			'[{"event_types":["FILE_CREATE","FILE_WRITE"],"conditions":[{"field":"path","op":"regex","value":"(/etc/shadow|/etc/passwd|\\.ssh/|id_rsa)"}],"label":"credential file accessed"},{"event_types":["NET_CONNECT"],"conditions":[{"field":"direction","op":"eq","value":"OUTBOUND"},{"field":"is_private","op":"eq","value":false}],"label":"outbound exfiltration"}]',
			60,
			'agent_id'
		)
		ON CONFLICT (id) DO NOTHING;
	...
	},
	{
		...
		VALUES
		(
			'rule-fim-critical-modified',
			'Critical System File Modified (FIM)',
			'A file monitored by the integrity checker was modified — possible unauthorized system change.',
			3,
			ARRAY['FIM_VIOLATION'],
			'[{"field":"action","op":"eq","value":"modified"}]',
			ARRAY['T1565.001'],
			'system',
			'match', 0, 0, ''
		),
		(
			'rule-fim-password-file',
			'Password File Modified (FIM)',
			'/etc/shadow or /etc/passwd was modified — possible account manipulation.',
			4,
			ARRAY['FIM_VIOLATION'],
			'[{"field":"action","op":"eq","value":"modified"},{"field":"file_path","op":"regex","value":"^/etc/(passwd|shadow|group|gshadow)$"}]',
			ARRAY['T1098'],
			'system',
			'match', 0, 0, ''
		),
		(
			'rule-fim-ssh-modified',
			'SSH Configuration Modified (FIM)',
			'SSH config or authorized_keys was modified �� possible persistence or backdoor.',
			4,
			ARRAY['FIM_VIOLATION'],
			'[{"field":"file_path","op":"regex","value":"(sshd_config|authorized_keys)"}]',
			ARRAY['T1098.004'],
			'system',
			'match', 0, 0, ''
		),
		(
			'rule-fim-deleted',
			'Monitored File Deleted (FIM)',
			'A file tracked by the integrity checker was deleted — possible evidence destruction.',
			3,
			ARRAY['FIM_VIOLATION'],
			'[{"field":"action","op":"eq","value":"deleted"}]',
			ARRAY['T1070'],
			'system',
			'match', 0, 0, ''
		),
		(
			'rule-fim-mass-changes',
			'Mass File Integrity Violations (FIM threshold)',
			'10+ FIM violations in 60 seconds — possible system-wide compromise or unauthorized mass change.',
			4,
			ARRAY['FIM_VIOLATION'],
			'[]',
			ARRAY['T1565.001'],
			'system',
			'threshold', 10, 60, 'agent_id'
		)
		ON CONFLICT (id) DO NOTHING;
		`,
	},
