...
// BrowserTreeNode represents a node in a URL navigation tree.
type BrowserTreeNode struct {
	...
	IsFormSubmit bool               `json:"is_form_submit"`
	...
	IOCMatch     *models.IOC        `json:"ioc_match,omitempty"`
	Children     []*BrowserTreeNode `json:"children,omitempty"`
}

// BrowserTab represents a distinct browser tab with its last known URL.
...
// browserNodeFromEvent extracts browser fields from an event's JSONB payload.
func browserNodeFromEvent(e *models.Event) *BrowserTreeNode {
	...
	node.URL = p.URL
	node.Domain = p.Domain
	node.Path = p.Path
	node.Method = p.Method
	node.StatusCode = p.StatusCode
	node.ResourceType = p.ResourceType
	node.Referrer = p.Referrer
	node.TabURL = p.TabURL
	node.TabID = p.TabID
	node.IsFormSubmit = p.IsFormSubmit
	node.Error = p.Error
	node.BrowserName = p.BrowserName
	node.ServerIP = p.ServerIP

	return node
}

// expandRedirectChain creates intermediate redirect nodes from the event payload.
func expandRedirectChain(e *models.Event) []*BrowserTreeNode {
	var p struct {
		RedirectChain []struct {
			URL        string `json:"url"`
			StatusCode int    `json:"statusCode"`
	...
	if err := json.Unmarshal(e.Payload, &p); err != nil || len(p.RedirectChain) == 0 {
		return nil
	}

	var nodes []*BrowserTreeNode
	for _, r := range p.RedirectChain {
		nodes = append(nodes, &BrowserTreeNode{
			URL:        r.URL,
...
// normalizeURL strips trailing slashes and fragments for matching purposes.
...
// GetBrowserTree builds a URL navigation tree from BROWSER_REQUEST events.
func (s *Store) GetBrowserTree(ctx context.Context, agentID string, since, until time.Time, tabID int) ([]*BrowserTreeNode, error) {
	q := `
		SELECT * FROM events
		WHERE agent_id = $1
		  AND event_type = 'BROWSER_REQUEST'
	...
	if tabID > 0 {
		q = `
			...
			WHERE agent_id = $1
			  AND event_type = 'BROWSER_REQUEST'
			  AND timestamp >= $2 AND timestamp <= $3
			  AND (payload->>'tab_id')::int = $4
			ORDER BY timestamp ASC
			LIMIT 2000
		`
		err = s.db.SelectContext(ctx, &events, q, agentID, since, until, tabID)
	} else {
		err = s.db.SelectContext(ctx, &events, q, agentID, since, until)
	}
	if err != nil {
		return nil, fmt.Errorf("browser tree query: %w", err)
	}
	if len(events) == 0 {
		return []*BrowserTreeNode{}, nil
	}

	// Load domain IOCs for matching.
	domainIOCs, _ := s.LoadActiveIOCs(ctx, "domain")

	// Build nodes from events.
	var allNodes []*BrowserTreeNode
	for i := range events {
		node := browserNodeFromEvent(&events[i])

		// Check domain IOC match.
		if ioc, ok := domainIOCs[strings.ToLower(node.Domain)]; ok {
			node.IOCMatch = ioc
		}

		// Expand redirect chain: attach as children of this node.
		redirectNodes := expandRedirectChain(&events[i])
		if len(redirectNodes) > 0 {
			node.Children = append(node.Children, redirectNodes...)
		}

		allNodes = append(allNodes, node)
	}

	// Index: normalized URL -> list of nodes (most recent main_frame preferred).
	urlIndex := make(map[string]*BrowserTreeNode)
	for _, n := range allNodes {
		norm := normalizeURL(n.URL)
		// Prefer main_frame nodes as parents.
		if existing, ok := urlIndex[norm]; ok {
			if n.ResourceType == "main_frame" && existing.ResourceType != "main_frame" {
				urlIndex[norm] = n
			}
		} else {
			urlIndex[norm] = n
	...
	// Track the most recent main_frame per tab for fallback parenting.
	lastMainFrame := make(map[int]*BrowserTreeNode)

	// First pass: record main_frame nodes per tab.
	for _, n := range allNodes {
		if n.ResourceType == "main_frame" {
			lastMainFrame[n.TabID] = n
		}
	}

	// Second pass: attach children.
	// Reset lastMainFrame and rebuild incrementally for correct ordering.
	lastMainFrame = make(map[int]*BrowserTreeNode)
	for _, n := range allNodes {
		if n.ResourceType == "main_frame" {
			lastMainFrame[n.TabID] = n
		...
		// Try to find parent by referrer.
		var parent *BrowserTreeNode
		if n.Referrer != "" {
			parent = urlIndex[normalizeURL(n.Referrer)]
		}
		// Fallback: try tab_url.
		if parent == nil && n.TabURL != "" {
			parent = urlIndex[normalizeURL(n.TabURL)]
		}
		// Fallback: attach sub-resources to the most recent main_frame for this tab.
		if parent == nil && n.ResourceType != "main_frame" {
			parent = lastMainFrame[n.TabID]
		}

		// Don't attach a node to itself.
		if parent != nil && parent.EventID != n.EventID {
			parent.Children = append(parent.Children, n)
			attached[n.EventID] = true
		}
	}

	// Collect root nodes: anything not attached as a child.
	var roots []*BrowserTreeNode
	for _, n := range allNodes {
		if !attached[n.EventID] {
...
// GetBrowserTabs returns distinct browser tabs with their last URL for an agent.
func (s *Store) GetBrowserTabs(ctx context.Context, agentID string, since time.Time) ([]BrowserTab, error) {
	q := `
		SELECT
			(payload->>'tab_id')::int AS tab_id,
			MAX(payload->>'tab_url') AS last_url,
		...
		WHERE agent_id = $1
		  AND event_type = 'BROWSER_REQUEST'
		  AND timestamp >= $2
		  AND payload->>'tab_id' IS NOT NULL
		GROUP BY (payload->>'tab_id')::int
	...
	var rows []row
	if err := s.db.SelectContext(ctx, &rows, q, agentID, since); err != nil {
		return nil, err
	}

	tabs := make([]BrowserTab, len(rows))
	for i, r := range rows {
		tabs[i] = BrowserTab{TabID: r.TabID, LastURL: r.LastURL, LastSeen: r.LastSeen, Count: r.Count}
	}
	return tabs, nil
}
